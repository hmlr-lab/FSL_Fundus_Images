...
#****************** Project PyILP Version 2 ******************#
#-------------------------------------------------------------
"""
PyILP A novel user-friendly Python/Jupyter interface for 
Inductive Logic programming(ILP) system for teaching 
relational machine learning and comparing different algorithms.

We have incoperated three different ILP algorithms in this packge ;
    1. Aleph
    2. Metagol
    3. Metagol_NT
...
developed by Ashwin Srinivasan: http://www.cs.ox.ac.uk/activities/machlearn/Aleph/

In this package we use a porting of Aleph v.5 to SWI-Prolog 
prepared by Fabrizio Riguzzi: https://github.com/friguzzi/aleph

##### Metagol
Metagol is an Meta-Interpretive Learning (MIL) system maintained 
by Andrew Cropper: https://github.com/metagol/metagol
...
def evaluate_theory_prolog_train(theory, file_1, pos, neg):
    file_2 = "copy_bk.pl"
    shutil.copyfile(file_1, file_2)
    file_object = open(file_2, 'a')
...
def metagolnt_learn(file="BK.pl", test_size=0.33, positive_example="pos_example.f",
                  negative_example="neg_example.n", shuffle=False, settings=[], num_pos=1, num_neg=1, verbose=False, max_run=2 ):
    start_time = time.time()
    ...
    if type(positive_example) == list:
        ...
        positive_example_ids = read_example_mil(positive_example)
    if type(negative_example) == list:
        negative_example_ids = negative_example
    else:
        negative_example_ids = read_example_mil(negative_example)
    if shuffle == True:
        random.shuffle(positive_example_ids)
        random.shuffle(negative_example_ids)
    length_positive_examples = len(positive_example_ids)
    ...
    positive_example_ids_test = positive_example_ids[positive_example_training_size:]
    negative_example_ids_test = negative_example_ids[negative_example_training_size:]
    H_2 = metagol_NT(num_pos, num_neg, final_bk_file, positive_example_ids_training, negative_example_ids_training, max_run)
    end_time = time.time() - start_time
    if test_size != 0:
        if H_2:
            #print(H_2)
            if not verbose:
                print(H_2)
                print("+----------+ Testing +----------+")
            test = evaluate_theory_prolog(H_2[0], file, positive_example_ids_test, negative_example_ids_test)
            #os.remove("theory.txt")
            # rint(test)
            return MIL(H_2[0], test.accuracy, test.precision, test.sensitivity, test.specificity, test.fscore, end_time)
    else:
        ...
        if positive_example and negative_example:
            if not verbose:
                print(H_2)
                print("+----------+ Learning +----------+")
            test = evaluate_theory_prolog(H_2[0], file, positive_example_ids_training, negative_example_ids_training)
            #os.remove("theory.txt")
            return MIL(H_2[0], test.accuracy, test.precision, test.sensitivity, test.specificity, test.fscore, end_time)
        else:
            os.remove("theory.txt")
...
def metagolnt_cross_validation(file="BK.pl", CV=2, positive_example="pos_example.f",
                             negative_example="neg_example.n", shuffle=False, settings=[], num_pos=1, num_neg=1, verbose=False, max_run=2):
    mil_cv_accuracy, mil_cv_precision, mil_cv_sensitivity, mil_cv_specificity, mil_cv_fscore = [], [], [], [], []
    ...
    for i in range(CV):
        ...
        #H_2 = metagol(final_bk_file, pos_train_fold, neg_train_fold)
        # H_2=H[0]
        #print(pos_train_fold, neg_train_fold)
        H_2 = metagol_NT(num_pos, num_neg, final_bk_file, pos_train_fold, neg_train_fold,max_run)
        print(H_2)
        if H_2:
            print("+----------+ Testing +----------+")
            test = evaluate_theory_prolog(H_2[0], file, pos_test_fold, neg_test_fold)
            #os.remove("theory.txt")
            mil_cv_accuracy.append(test.accuracy)
            mil_cv_precision.append(test.precision)
            mil_cv_sensitivity.append(test.sensitivity)
...
def read_example_mil(file_name):
    file_1 = open(file_name, 'r')
    lines = file_1.readlines()
    list_1 = []
    for eachline in lines:
        line_1 = eachline.strip()
        if line_1 and line_1[0] != "%":
            list_1.append(line_1.strip()[0:-1])
    file_1.close()
    return list_1


def metagol_NT(pos_sample, neg_sample, file_name,pos, neg,  max_run):
    run=1
    score=0
    H=[]
    while run<=max_run:
        ...
        #print(run)
        random.seed(run)
        pos_ex_list=random.sample(pos, k=pos_sample)
        neg_ex_list=random.sample(neg, k=neg_sample)
        pos_ex_list_1=[]
        neg_ex_list_1=[]
        for i in pos_ex_list:
            if "." in i:
                string_1=i[0:-1]
                #print(string_1)
                pos_ex_list_1.append(string_1)
            else:
                pos_ex_list_1.append(i)
                
        for i in neg_ex_list:
            if "." in i:
                string_1=i[0:-1]
                #print(string_1)
                neg_ex_list_1.append(string_1)
            else:
                neg_ex_list_1.append(i)        
                
                
        pos_ex_list=pos_ex_list_1
        neg_ex_list=neg_ex_list_1
        
        pos_ex_list_val=list(np.setdiff1d(pos, pos_ex_list))
        neg_ex_list_val=list(np.setdiff1d(neg, neg_ex_list))
        #print(pos_ex_list, pos_ex_list_val)
        string_1=":- Pos=["+",".join(pos_ex_list)+"],Neg =["+",".join(neg_ex_list)+"], learn(Pos,Neg)."
        file_1="processed_bk.pl"
        shutil.copyfile(file_name, file_1)
        file_object = open(file_1, 'a')
        ...
        #print(theory)
        if theory:
            metrics=evaluate_theory_prolog_train(theory, file_name,pos_ex_list_val, neg_ex_list_val)#, verbose=True
            if metrics.accuracy>score:
